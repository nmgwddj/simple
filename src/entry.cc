...
int fts5_simple_xCreate(void* sqlite3,
                        ...
                        Fts5Tokenizer** ppOut) {
    int rc = SQLITE_OK;
    simple_tokenizer::SimpleTokenizer* p =
        new simple_tokenizer::SimpleTokenizer(azArg, nArg);
...
static void jieba_dict(sqlite3_context* pCtx, int nVal, sqlite3_value** apVal) {
    if (nVal >= 1) {
        ...
        if (text) {
            ...
            simple_tokenizer::jieba_dict_path__ = tmp;
...
int sqlite3_simple_init(sqlite3* db,
    ...
    rc = sqlite3_create_function(db, "query", -1,
